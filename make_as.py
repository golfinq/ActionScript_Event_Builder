...
class ActionScriptMaker:
    ...
    def _make_sig(self):
        ...
        self.doc_url = str(PurePosixPath(self.file_path)).replace("doc_trunk/doc_pages/",
...
def write_code(maker: ActionScriptMaker):
    ...
    class_prop_lines = []
    for c in maker.class_props.get_properties_by_con_order():
        class_prop_lines.extend([f"// {c.desc}",
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Make ActionScript Classes based on parsed documentation - roughly")
    parser.add_argument("--ignore", "-i", action="store_true", help="ignore existing files")
    parser.add_argument("--output", "-o", default="as_gen", help="output generated path; Default: `./as_gen`")
    parser.add_argument(
        "--include-air", "-a",
        action="store_true",
...
def main():
    ...
    for path_ele in args.path.split("."):
        search_path = Path(path_ele) if (search_path is None) else (search_path / path_ele)
    ...
    for html_file in input_files:
        ...
        try:
            ...
            continue
        (gen_path / f"{class_name}.as").write_text(remove_multiple_newlines(write_code(asm_maker).strip()))
        print(
